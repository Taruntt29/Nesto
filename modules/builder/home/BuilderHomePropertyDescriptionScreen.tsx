...
import { screenWidth, vh, vw, normalize } from '@nestoHub/utils/dimensions';
...
import { getAllCurrentPackage, getAllFloorPlansAndPricing, getPropertyById } from '@nestoHub/actions';
...
export default function BuilderHomePropertyDescriptionScreen(props: Props) {
  ...
  const [currentPackageData, setCurrentPackageData] = useState<any>([]);
  const [isVisible, setIsVisible] = useState<boolean>(false);
  const [selectPropertiesData, setSelectPropertiesData] = useState<any>({ count: '', data: [] })

  useEffect(() => {
    let payload = {
      id: props?.route?.params?.propertyId?._id ?? ''
    }
  ...
  useEffect(() => {
    dispatch(getAllFloorPlansAndPricing({ id: props?.route?.params?.propertyId?._id ?? '', unitType: availableFlatTypes?.filter((item: any) => item?.isSelected)?.map((item: any) => item?.title ?? '') }, (data: any) => {
  ...
  const propertyAnalytics = [
    {
      icon: localImages.SOLD,
      subTitle: '2',
      title: 'Sold',
    },
    {
      icon: localImages.VISIT,
      subTitle: '20',
      title: 'Visits',
    },
    {
      icon: localImages.SEARCHICON,
      subTitle: '200',
      title: 'Search',
    },
    {
      icon: localImages.VIEW,
      subTitle: '100',
      title: 'Views',
    },
    {
      icon: localImages.SHARE,
      subTitle: '40',
      title: 'Shared',
    },
  ];
  ...
  const FlatsView = memo(() => {
    return (
      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        {/* <View style={styles.priceVisitView}><Text style={styles.priceVisitText}>Get upto ₹{propertyData?.visitBrokerage ?? ''} Per Visit</Text></View> */}
      </View>
    )
  })
    ...
    return (
      <View style={styles.propertyLabelView}>
        ...
        {icon ? (
          <FastImage
            source={icon}
            style={styles.propertyLabelImage}
            resizeMode={'contain'}
          />
        ) : null}
      </View>
    ...
    return (
      <>
        <TouchableOpacity style={styles.bigImgHolder} activeOpacity={0.8} onPress={onPressBigImage}>
          ...
          <BrokerBlueLabel labelText={`${propertyData?.brokerageValue ?? ''}% Brokerage`} extraStyle={styles?.brokerageStyle} />
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: vh(8.2) }}>
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            {propertyData?.constructionStatus?.length ? <PropertyStatusLabel label={propertyData?.constructionStatus ?? ''} /> : null}
            {propertyData?.possessionDate?.length ? <PropertyStatusLabel label={propertyData?.possessionDate ?? ''} /> : null}
          </View>
    ...
    return (
      <View>
        ...
        <View style={styles.brokerageCard}>
          <Text style={styles.brokerageText1}>Brokerage 5%</Text>
          <Text
            style={
              styles.brokerageText2
            }>{`Platform charges & applicable taxes shall be deducted`}</Text>
        </View>
      </View>
    );
  });
  ...
  const PriceRange = memo((props: Props1) => {
    const onPress = () => {
      props?.navigation.navigate(screenNames?.BUILDER_PROPERTY_EDIT_REQUEST, { propertyData });
    };
    return (
      <View style={styles.box}>
        <Text style={styles.title}>{props.priceRange}</Text>
        <View style={styles.btn}>
          <Text
            style={
              styles.subTitle
            }>{`Book now & get ${props?.discount} discount`}</Text>
          <TouchableOpacity onPress={onPress} style={styles.Btn2}>
            <FastImage source={localImages.EDIT_PENCIL} style={styles.edit} />
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView
        ...
        <FlatsView />
        <PriceRange
          priceRange={`₹ ${propertyData?.minPrice}- ${propertyData?.maxPrice}`}
          discount={propertyData?.discountDescription}
          navigation={props?.navigation}
        />
        <BrokerPropertyHorizontalCard
          title="Property Analytics"
          itemList={propertyAnalytics}
          iconStyle={styles?.amenitiesIcon}
          extraTextStyle={styles?.propertyAnalyticsExtraTextStyle}
        />
        <Text style={styles.titleText}>Current Subscription</Text>
        {[currentPackageData?.[0]]?.map((item: any) => {
          return (
            <PackagePlanComp
        ...
        <AboutView
          ...
          onPressViewDetails={() => {
            props?.navigation?.navigate(screenNames?.BUILDER_ABOUT_THE_SCREEN, {
              title: 'Builder',
            });
          }}
...
const styles = StyleSheet.create({
  ...
  propertyAnalyticsExtraTextStyle: {
    fontSize: vw(11),
    fontWeight: '400',
    color: colors.black,
    fontFamily: fonts?.BAHNSCHRIFT,
