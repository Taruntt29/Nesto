...
export default function BuilderAllVisitScreen(props: Props) {
  ...
  useEffect(() => {
    dispatch(getAllVisit({ builderId: _id, propertyId: props?.route?.params?.propertyId }, (data: any) => {
  ...
  return (
    <KeyboardAwareScrollView contentContainerStyle={styles.mainContainer} showsVerticalScrollIndicator={false} bounces={false}>
      <BuilderVisitComponent statsData={visitAnalyticsData} tableData={visitData} type={1} navigation={props?.navigation} />
