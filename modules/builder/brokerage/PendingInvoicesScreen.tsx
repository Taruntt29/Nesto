...
export default function PendingInvoicesScreen(props: Props) {
    const dispatch: any = useDispatch()

    const { _id } = useSelector((state: any) => state?.authReducer)
    const { allBuilderPendingInvoices } = useSelector((state: any) => state?.builderHomeReducer)
    ...
    }, [allBuilderPendingInvoices])

    const keyExtractor = useCallback((item: any, index: any) => index?.toString(), [allBuilderPendingInvoices])
    ...
    return (
        <KeyboardAwareScrollView contentContainerStyle={styles.mainContainer} showsVerticalScrollIndicator={false} bounces={false}>
            ...
            <FlashList
                ...
                data={allBuilderPendingInvoices ?? []}
